...
{#if rows.length > 0}
	...
	<div class="{rounded} bg-gray-900/10 p-4 {className}">
		<div class="flex flex-col gap-3">
			{#each processedRows as row}
				{#if row.type === 'full'}
					<div class="{row.height} animate-pulse rounded bg-gray-900/15"></div>
...
{:else}
	<!-- Simple skeleton -->
	<div class="{height} {width} {rounded} animate-pulse bg-gray-900/10 {className}"></div>
{/if}
