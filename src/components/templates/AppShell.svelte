...
<div class="flex min-h-screen flex-col bg-neutral-100 {className}">
	<!-- Sticky Header -->
	<header class="sticky top-0 z-40 h-14 bg-white px-4 py-2 shadow-lg">
			...
			<div class="flex gap-4">
				...
				>
					<SvgIcon
						name={logoIcon}
						size="w-8 h-8"
						className="text-azure-700 group-hover:text-azure-900 transition-colors"
					/>
				</a>
				<div class="group hidden items-center justify-center gap-2 sm:flex">
					{#if title}
						<h1
							class="text-center text-sm leading-none font-semibold whitespace-nowrap text-azure-700 lg:text-xl"
						>
							/ {title}
						</h1>
					{/if}
					{#if versionString}
						<p class="hidden p-0.5 px-1 text-[10px] font-semibold text-gray-700 sm:inline">
							{versionString}
						</p>
					{/if}
				</div>
